...
SaveResultsTableAllCombinationTraits <- function(Network,Direction){
    ...
    for(i in 1:dim(ListTraits)[1]){
        ...
        # categorical trait
            ...
            Y<-as.data.frame(read.csv(paste("../data/",Network,"/traits/",ListTraits[i,2],sep=""),header=FALSE, sep=" "))

            Y<-Y[-removeSp,]
            Y<-data.frame(sp=rownames(adj),t=Y)
            Y<-Y[match(tree$tip.label, Y$sp),]
            Y<-Y$t
            Y<-data.frame(row.names=tree$tip.label, t=Y)

            # just to make sure the categorical trait is counted as catecorical even if it is listed as a number
            Y[,1]<-as.character(Y[,1])
            # calculate the phylogeneically-corrected traits
            U_cat <- phylo.correction(Y,tree)
            assign(paste0("corr_", ListTraits[i,1]), U_cat)
        }
    }
    
    
    # combining the corrected traits into a list
    # must be a better way doing this...
    
    # A list consisting of all phylogenetic corrected traits
